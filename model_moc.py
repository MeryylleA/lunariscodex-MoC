...
class ReasoningFeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # Initialize thought state as a clone of x to keep same dtype/shape
        h = x.clone()
...
class MoCTopKExperts(nn.Module):
    ...
    def __init__(self, config: LunarisCodexConfig):
        ...
        # Experts (now use ReasoningFeedForward)
...
class Block(nn.Module):
    def __init__(self, config: LunarisCodexConfig):
        ...
        if config.n_experts is not None and config.n_experts > 0 and config.top_k >= 1:
            ...
            # Fall back to single-expert FFN with reasoning loop as well for consistency
