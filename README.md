...
3. **Thought State Evolution**: The internal state `h` is updated through the SwiGLU MLP at each reasoning step
4. **Computational Efficiency**: Despite multiple steps, the parameter count remains the same as traditional FFNs

```python
def forward(self, x: torch.Tensor):
    # Initialize thought state as a clone of x
    h = x.clone()
    steps = max(1, self.n_reasoning_steps)
    for _ in range(steps):
        # Residual injection of original x at every step
        h = self._ffn_logic(h + x)
    return h
```
...
- **Stable Training**: Residual connections at each step ensure gradient flow and training stability
